...
interface Props {
    active: 'dashboard' | 'history' | 'kegiatan';
        ...
        <>
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-gray-100 flex flex-col justify-between shadow-xl md:shadow-[4px_0_24px_rgba(0,0,0,0.02)] flex-shrink-0 transition-transform duration-300 ease-in-out md:static md:translate-x-0 ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                ...
                <nav className="mt-8 px-4 space-y-2">
                    ...
                    <Link href={route('driver.kegiatan')} className={`flex items-center px-4 py-3 rounded-2xl transition-all ${active === 'kegiatan' ? 'text-green-700 bg-green-50/80 font-semibold shadow-sm' : 'text-gray-500 hover:bg-gray-50 hover:text-green-600 font-medium group'}`}>
                        <span className={active !== 'kegiatan' ? 'group-hover:text-green-500 transition-colors' : ''}><ActivitiesIcon /></span> Kegiatan
                    </Link>
