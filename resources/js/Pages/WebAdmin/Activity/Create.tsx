...
export default function Create() {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('webadmin.activities.store'));
    };

    return (
        <div className="flex h-screen bg-[#F8FAFC] font-sans">
            <Head title="Tambah Aktivitas | Web Admin" />
            ...
            <div className="flex-1 flex flex-col overflow-hidden relative">
                <header className="h-24 flex items-center justify-between px-6 sm:px-10 bg-white/60 backdrop-blur-md border-b border-gray-100 z-10 sticky top-0">
                    <div className="flex items-center gap-4 flex-1">
                        ...
                        <h1 className="text-xl font-bold text-gray-800">Tambah Aktivitas Baru</h1>
                    </div>
                </header>
                ...
                <main className="flex-1 overflow-x-hidden overflow-y-auto px-6 py-8 sm:px-10">
                    <div className="max-w-3xl bg-white rounded-3xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 p-8">
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-2">Gambar Publikasi (Opsional)</label>
                                <input 
                                    type="file" 
                                    accept="image/*"
                            ...
                            <div className="flex justify-end space-x-4 pt-4 border-t border-gray-100">
                                ...
                                <button disabled={processing} type="submit" className="px-6 py-3 bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl shadow-sm transition-colors disabled:opacity-50">
                                    Simpan Aktivitas
                                </button>
                            </div>
                        </form>
