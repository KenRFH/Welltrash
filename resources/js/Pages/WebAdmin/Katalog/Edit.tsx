...
export default function Edit({ katalog }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('webadmin.katalogs.update', katalog.id));
    };

    return (
        <div className="flex h-screen bg-[#F8FAFC] font-sans">
            <Head title="Edit Katalog | Web Admin" />
            ...
            <div className="flex-1 flex flex-col overflow-hidden relative">
                <header className="h-24 flex items-center justify-between px-6 sm:px-10 bg-white/60 backdrop-blur-md border-b border-gray-100 z-10 sticky top-0">
                    <div className="flex items-center gap-4 flex-1">
                        ...
                        <h1 className="text-xl font-bold text-gray-800">Edit Item Katalog</h1>
                    </div>
                </header>
                ...
                <main className="flex-1 overflow-x-hidden overflow-y-auto px-6 py-8 sm:px-10">
                    <div className="max-w-3xl bg-white rounded-3xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 p-8">
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-2">Nama Item</label>
                                <input 
                                    type="text" 
                                    value={data.name}
                            ...
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-2">Ganti Gambar Produk (Opsional)</label>
                                {katalog.image_path && (
                                    <div className="mb-3">
                                        <img src={`/storage/${katalog.image_path}`} alt="Preview" className="h-32 rounded-lg object-cover" />
                                    </div>
                                )}
                                <input 
                                    type="file" 
                            ...
                            <div className="flex justify-end space-x-4 pt-4 border-t border-gray-100">
                                <Link href={route('webadmin.katalogs.index')} className="px-6 py-3 text-gray-600 font-semibold hover:bg-gray-50 rounded-xl transition-colors">Batal</Link>
