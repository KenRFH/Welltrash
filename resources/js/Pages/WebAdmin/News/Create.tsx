...
export default function Create() {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('webadmin.news.store'));
    };

    return (
        <div className="flex h-screen bg-[#F8FAFC] font-sans">
            <Head title="Tulis Artikel Baru | Web Admin" />
                ...
                <main className="flex-1 overflow-x-hidden overflow-y-auto px-6 py-8 sm:px-10">
                    <div className="max-w-4xl bg-white rounded-3xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 p-8">
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-2">Gambar Header (Opsional)</label>
                                <input 
                                    type="file" 
                                    accept="image/*"
